...
from PyQt5.QtWidgets import QMainWindow, QApplication, QFileDialog, QTextEdit, QPlainTextEdit, QPushButton, \
    QRadioButton, QComboBox, QSpinBox, QDoubleSpinBox, QTreeView, QFileSystemModel
from PyQt5.QtCore import pyqtSignal, QDir, QSortFilterProxyModel, QThreadPool
...
class Ui(QMainWindow):
    ...
    def Database_browsing(self):
        ...
        if Data_Path != "":
            shutil.copy2(Data_Path, './Datasets')
    ...
    def Model_browsing(self):
        ...
        if Data_Path != "":
            shutil.copy2(Data_Path, './Models/Input Models/')
    ...
    def processClicked(self):
        ...
        elif self.SaveName.toPlainText() in os.listdir(
                os.getcwd() +os.sep+ 'results') or self.SaveName.toPlainText() in os.listdir(os.getcwd() + os.sep+ 'results'):
        ...
        if len(self.View_Model.selectedIndexes()) == 0:
        ...
        elif self.Pruning.isChecked() is False and self.Quantaziation.isChecked() is False and self.Distilled_Knowledge.isChecked() is False:
            self.Console.appendPlainText('\n###Please select a method###\n')
        
        elif os.path.exists(self.Teacher_Model_Path.toPlainText()) is False and self.Distilled_Knowledge.isChecked():
            ...
            if len(self.View_Database.selectedIndexes()) != 0:
                current_dataset_path = self.dirdataset.fileInfo(
                    self.View_Database.selectedIndexes()[0]).absoluteFilePath()
            
            current_model_path = self.dirmodel.fileInfo(self.View_Model.selectedIndexes()[0]).absoluteFilePath()
            
            if current_model_path.endswith('.zip'):
                with zipfile.ZipFile(current_model_path) as zip_file:
                    zip_file.extractall('./Models/Temp')
                zipselector = ZipSelector()
                zipselector.exec_()
                model = zipselector.getSelected()
                ...
                if model == "":
                    ...
                    current_model_path = os.path.join('./Models/Input models', model)
                    self.Console.appendPlainText('\n###Model extracted###\n')
            ...
            elif current_model_path.endswith('.pt'):
                framework = 'torch'
            
            worker_id = self.worker_id_counter
            self.worker_id_counter += 1
            progresswindow = self.create_progressWindow(current_model_path, 'results/' + save_name, worker_id)
            
            dictio = {'console': self.Console, 'model_path': current_model_path,
                      'Pruning': self.Pruning.isChecked(),
                      'Quantization': self.Quantaziation.isChecked(),
                      'Knowledge_Distillation': self.Distilled_Knowledge.isChecked(),
                      'batch_size': self.BatchSizeSB.value(),
                      'pruning_ratio': self.PruningRatioSB.value(), 'dataset_path': current_dataset_path,
